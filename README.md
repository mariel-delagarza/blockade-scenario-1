...
- Step 3: in `index.html`, if you're adding multiple markers _**that use the same gif**_ and adding/removing them at the same time, you can follow the code that's in this project's `index.html` file (starting on line 365). You'll make a list to hold your markers, make a list of their locations (the lat/long pairs from Step 2), and then create the `div`s that will hold your markers, tell them which gifs to use, and tell them where on the map they go.
